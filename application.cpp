...
GLuint loadShader(const char* vertexfilename, const char* fragmentfilename)
{
    ...
    string vs = getTextFile(vertexfilename);
    int size_v = vs.length();
    
    char *ptr_v = &vs[0];
    
    glShaderSource(vertex_shader_id, 1, &ptr_v,  &size_v);
    glCompileShader(vertex_shader_id);
    glAttachShader(program, vertex_shader_id);
    
    fragment_shader_id = glCreateShader(GL_FRAGMENT_SHADER);
    string fs = getTextFile(fragmentfilename);
    int size_f = fs.length();
    
    char *ptr_f = &fs[0];
    
    glShaderSource(fragment_shader_id, 1, &ptr_f, &size_f); 
